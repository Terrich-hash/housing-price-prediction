...
# Function: Input and predict
def customer_input_and_predict(model):
    print("\n--- Predict House Price ---")
    try:
        area = float(input("Enter the area of the house (in sq ft): "))
        bedrooms = int(input("Enter the number of bedrooms: "))
        predicted_price = predict_price(area, bedrooms, model)
        print(f"\nEstimated Price: ${predicted_price:,.2f}")
    except ValueError:
        print("Invalid input. Please enter numeric values for area and bedrooms.")
...
# Take input from the customer and predict the price
customer_input_and_predict(trained_model)